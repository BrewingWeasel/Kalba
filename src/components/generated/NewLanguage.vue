   ...
   <DialogScrollContent>
      ...
      <div class="">
        <DialogClose as-child v-for="[language, tags] in languages">
           <div @click="$emit('langSelected', language)" class="flex items-center p-2 rounded-md hover:bg-accent">
              <h2 class="grow">{{ language }}</h2>
              <Badge v-for="tag in tags" class="m-1" :class="tagColors.get(tag)">{{tag}}</Badge>
           </div>
        </DialogClose>
      </div>
   </DialogScrollContent>
